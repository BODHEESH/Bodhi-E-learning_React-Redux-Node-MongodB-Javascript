...
function CourseList() {
    ...
    const savehandler = (SID) => {
        ...
        try {
            const res = axios.put(`/course/savecourse/${SID} `, { userId: currentUser._id });
                                    ...
                                    <div class="flex justify-between space-x-2">
...

                                        {
                                            obj.enrollments.includes(currentUser._id) ?
                                                ...
                                                :
                                                <a class="font-semibold text-sm inline-flex items-center justify-center px-3 py-1.5 border border-transparent rounded leading-5 shadow-sm transition duration-150 ease-in-out bg-indigo-500 focus:outline-none focus-visible:ring-2 hover:bg-green-600 text-white" href="/courseview" onClick={() => enrollhandler(obj._id)}>Enroll Now</a>
                                        }
                                    </div>
                                </div>
                            </div>
