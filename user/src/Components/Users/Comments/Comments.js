    ...
    <div>
      <form className="flex items-center py-2" onSubmit={handleComment}>
        ...
        <Link to={`/profile/${currentUser.username}`}>
          <img
            class="w-8 h-8 ml-2 rounded-full bg-gray-100"
        ...
        <textarea
          ...
          className="border-none flex-1 focus:ring-0 outline-none flex flex-wrap mx-5"
      ...
      <p className="text-right text-gray-400 text-sm" onClick={handleShow}>View All Comments</p>
      {seeComments.map((obj) => {
        return (
          <>
            ...
            {commentShow ? (
              <div className="comment ">
                <div className="w-32  ">
                  ...
                  <div className="flex justify-center">
                    {obj.userId.username === currentUser.username ?
                      <p className='font-semibold px-2 mt-1 flex justify-center'>You</p>
                      :
                      <p className='date px-2 mt-1 flex justify-center'>{obj.userId.username}</p>}
                  </div>
                ...
                <span className="date w-20">{format(obj.createdAt)}</span>
              </div>
            ) : null}
          </>
        );
